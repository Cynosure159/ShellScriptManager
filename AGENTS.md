...
- **主题**: 暗色主题
- **主题色**: 低饱和度紫色 `#8B7EC8`
- **Shell 类型**: Bash、Batch、PowerShell
...
- **平台适配**: 脚本类型选项根据平台动态显示 (Windows: Batch/PowerShell, Mac/Linux: Bash)。
- **编辑器**: 增加了自动换行开关。
- **UI 增强**: 列表侧边栏宽度支持拖拽调整，分类侧边栏支持折叠收起（悬浮半圆拉手设计）。
- **导入增强**: 支持导入单个脚本文件 (.sh/.bat/.ps1)，并自动以文件名命名。
- **终端导出**: 支持将终端输出保存为文件，并自动建议文件名。
...
2. **健壮性**: 优化 IPC 文件写入的错误处理机制。
3. **多语言支持**: 引入 i18n 支持并国际化界面文案。
4. **日志系统**: 完善日志记录并支持导出。
5. **工程化**: 配置 GitHub Actions CI/CD。
